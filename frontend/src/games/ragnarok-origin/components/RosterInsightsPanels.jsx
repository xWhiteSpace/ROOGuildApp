...
function useRosterJobInsights(members, jobsCatalog) {
  const jobDistributionTally = useMemo(() => {
    const tally = {};
    Object.values(members || {}).forEach((m) => {
      if (m.isRaidRoster === true && m.jobCode) {
        tally[m.jobCode] = (tally[m.jobCode] || 0) + 1;
      }
    });
    return tally;
  }, [members]);
  ...
  const totalRaiders = useMemo(
    () => Object.values(jobDistributionTally).reduce((sum, val) => sum + val, 0),
    [jobDistributionTally]
  );
  ...
  const conicGradientString = useMemo(() => {
    ...
    return gradientSegments.length > 0
      ? `conic-gradient(${gradientSegments.join(', ')})`
      : 'conic-gradient(#1e293b 0% 100%)';
  }, [jobsCatalog, jobDistributionTally, totalRaiders]);

  const maxMemberRows = useMemo(
    () => Math.max(0, ...Object.values(membersByJob).map((l) => l.length)),
    [membersByJob]
  );

  return { jobDistributionTally, membersByJob, totalRaiders, conicGradientString, maxMemberRows };
}

function CompositionBalancingPanel({ jobsCatalog, jobDistributionTally, isOfficer, onUpdateDesiredTarget }) {
  return (
                  ...
                  <tr key={code} className="hover:bg-slate-900/10 transition-colors">
                    ...
                    <td className="p-2 text-center">
                      <input
                        ...
                        disabled={!isOfficer}
                        onChange={(e) => onUpdateDesiredTarget(code, e.target.value)}
...
function ClassDensityPanel({ jobsCatalog, jobDistributionTally, totalRaiders, conicGradientString }) {
  return (
    <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 space-y-5 shadow-xl flex flex-col items-center">
      ...
      <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-2 pt-2">
        {Object.entries(jobsCatalog).map(([code, jobObj]) => {
          const activeCount = jobDistributionTally[code] || 0;
...
function RosterByJobPanel({ jobsCatalog, membersByJob, maxMemberRows }) {
  return (
    <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl">
      <div className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 select-none pb-2 border-b border-slate-900">
...
export default function RosterInsightsPanels({ panel, jobsCatalog, members, isOfficer, onUpdateDesiredTarget }) {
  const { jobDistributionTally, membersByJob, totalRaiders, conicGradientString, maxMemberRows } = useRosterJobInsights(members, jobsCatalog);

  if (panel === 'balancing') {
    return (
      <CompositionBalancingPanel
        jobsCatalog={jobsCatalog}
        jobDistributionTally={jobDistributionTally}
        isOfficer={isOfficer}
        onUpdateDesiredTarget={onUpdateDesiredTarget}
      />
    );
  }
  ...
  if (panel === 'density') {
    return (
      <ClassDensityPanel
        jobsCatalog={jobsCatalog}
        jobDistributionTally={jobDistributionTally}
        totalRaiders={totalRaiders}
        conicGradientString={conicGradientString}
      />
    );
  }
