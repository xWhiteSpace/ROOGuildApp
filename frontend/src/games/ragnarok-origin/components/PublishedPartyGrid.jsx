...
export default function PublishedPartyGrid({
  ...
  const canEdit = isOfficer && !readOnly;
  ...
  const persistGrids = async (grids) => {
    if (!publishedId || !canEdit) return;
    setSaving(true);
    try {
      if (typeof persistFn === 'function') {
        await persistFn(grids);
        isDirtyRef.current = false;
        setIsDirty(false);
        return;
      }
  ...
  const handleCellDropIntercept = (e, destCoordKey) => {
    e.preventDefault();
    if (!canEdit || !activeTabConfigId) return;
  ...
  const rosterPanel = (
    <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 shadow-md h-[17.5rem] min-h-0 overflow-hidden">
      <RosterSidebar
  ...
  const gridCanvas = (
    <div className="border border-slate-800 bg-slate-950 rounded-2xl p-4 min-h-[42rem] relative">
        ...
        <div className="grid gap-2 p-2" style={{ gridTemplateColumns: `repeat(${columnsCount}, minmax(130px, 1fr))` }}>
          ...
          {Array.from({ length: columnsCount }).map((_, cIdx) => (
            <div key={`h-${cIdx}`} className="px-1 py-1 border-b border-slate-900 pb-2">
              <input type="text" disabled={!canEdit} value={slots[`party_name_${cIdx + 1}`] || `Party ${cIdx + 1}`} onChange={(e) => handleUpdatePartyName(cIdx + 1, e.target.value)} className="w-full text-center bg-transparent text-[10px] font-mono font-black uppercase text-slate-400 outline-none" />
            </div>
          ))}
          {Array.from({ length: rowsCount }).flatMap((_, rIdx) =>
            Array.from({ length: columnsCount }).map((_, cIdx) => {
              ...
              return (
                <div
                  key={coordKey}
                  draggable={canEdit && !!slotData.userId}
                  onDragStart={(e) => { if (canEdit && slotData.userId) e.dataTransfer.setData('text/plain', JSON.stringify({ source: 'cell', coordKey, userId: slotData.userId })); }}
                ...
                >
                  ...
                  <div className="flex-1 cursor-pointer pt-3" onClick={() => { if (!canEdit) return; setActivePopover(isAssignPopoverOpen ? null : { coordKey, type: 'assign' }); }}>
                    {allocatedUserObj ? (
                      <RaidMemberCard allocatedUserObj={allocatedUserObj} jobObj={jobsCatalog[allocatedUserObj.jobCode]} currentStatus={commitments[calendarSignKey]?.[slotData.userId]?.status} isVoiceActive={liveVoiceUids.includes(slotData.userId)} isPartyLeader={isPartyLeader} isRaidLeader={isRaidLeader} />
                    ) : (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-12 gap-4 items-stretch">
        <div className={`${rightPanelCollapsed ? 'col-span-12' : 'col-span-12 xl:col-span-9'} pt-8`}>
          {gridCanvas}
        </div>
        {rightPanelCollapsed ? (
